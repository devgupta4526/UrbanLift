package com.example.Uber_LocationService.api;
 ...
 * Plus {@code 503} for Redis {@link org.springframework.dao.DataAccessException}.
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> dataAccess(DataAccessException ex) {
        return error(HttpStatus.SERVICE_UNAVAILABLE, "Location store temporarily unavailable");
    }

    @ExceptionHandler(DataIntegrityViolationException.class)
    public ResponseEntity<Map<String, Object>> dataIntegrity(DataIntegrityViolationException ex) {
        log.warn("Data integrity violation: {}", ex.getMostSpecificCause().getMessage());
        return error(HttpStatus.CONFLICT, "Request conflicts with stored location data");
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<Map<String, Object>> fallback(Exception ex) {
        log.error("Unhandled error in Location service", ex);
